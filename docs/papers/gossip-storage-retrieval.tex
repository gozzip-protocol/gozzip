...
This is a mean-field approximation that assumes uniform random neighborhood overlap. In highly clustered networks ($C > 0.5$), actual unique reach is substantially lower because 2-hop neighborhoods overlap heavily. At realistic online rates (${\sim}46\%$), the unique \emph{online} reach is approximately 400--500 nodes. The protocol compensates through WoT-targeted forwarding (gossip is directed at likely holders, not broadcast randomly) and relay fallback (Tier~4) for the estimated 5--10\% of reads that gossip alone cannot serve.
...
\textbf{Redundancy} & Manual multi-relay publishing & None---single home instance & Equilibrium-seeking pact count + 3 standby; availability 99.9--99.97\% under realistic correlated failures ($10^{-9}$ under theoretical independent-failure assumption) \\
