...
\begin{enumerate}
  ...
  \item An \textbf{iroh transport architecture} that gives each node direct QUIC connectivity with Ed25519 endpoint identity and relay-assisted hole punching, extensible to mesh radio, Bluetooth, and Tor multipath (Section~\ref{sec:iroh}).
\end{enumerate}
...
\textbf{Tier~2---Partner query (kind 10057):} The node sends a NIP-44-encrypted direct data request to one of the author's known pact partners, addressed over iroh (Section~\ref{sec:iroh}). The request is signed and routed by pubkey; the partner responds with a data offer (kind 10058). Latency: ${\sim}80$ms + jitter.
...
Relay-mediated channels do not replace the retrieval cascade (Section~\ref{sec:retrieval}) but provide a reliable encrypted communication substrate for all peer-to-peer protocol messages. They complement the iroh transport (Section~\ref{sec:iroh}) for internet-based connectivity, offering an additional path that works through the existing Nostr relay network when direct connections are unavailable.
...
\begin{itemize}
  ...
  \item \textbf{iroh}~\cite{iroh} --- The maintained peer-to-peer QUIC transport the protocol builds on: direct, identity-addressed connections with relay-assisted hole punching and connection migration, extensible to future mesh/radio/Tor multipath.
