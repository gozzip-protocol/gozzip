...
For a 5,000-node network with $k=20$ and $C=0.25$: $\text{reach}(1) = 20$, $\text{reach}(2) = 300$, $\text{reach}(3) \approx 4{,}500$. Three hops theoretically cover 90\%+ of the network. In practice, however, simulation across multiple graph topologies (Barab\'{a}si--Albert, Watts--Strogatz, LFR benchmark) shows that gossip delivers only 0.1--9\% of reads; the vast majority (74--92\%) are served instantly from local pact storage. The protocol's effectiveness derives primarily from pact-local reads, not gossip propagation. Gossip serves as a secondary delivery mechanism for content not yet cached locally, while relay fallback (Tier~4) handles an additional 6--21\% of reads depending on topology and network maturity. Increasing TTL to 4 would add minimal reach at substantial bandwidth cost---the logarithmic path length means most information arrives within 2--3 hops.
...
\textbf{Graph density and protocol performance:} Simulation reveals a counterintuitive result---sparser BA graphs ($m{=}10$, average degree ${\sim}20$) achieve higher availability (98.4\%) than denser graphs ($m{=}50$, average degree ${\sim}100$) at 95.3\%. Sparser graphs produce more pronounced hub structure, which improves pact partner diversity and reduces correlated failures. Watts--Strogatz small-world graphs (97.7\%) fall between the two, with higher gossip utilization (9.3\% of reads vs.\ $<$0.1\% for BA) compensating for less pronounced hub structure.
...
This is a mean-field approximation that assumes uniform random neighborhood overlap. In highly clustered networks ($C > 0.5$), actual unique reach is substantially lower because 2-hop neighborhoods overlap heavily. At realistic online rates (${\sim}46\%$), the unique \emph{online} reach is approximately 400--500 nodes. Simulation confirms that gossip delivers only 0.1--9.3\% of reads across tested topologies; the protocol's primary delivery mechanism is pact-local instant reads (74--92\%), not gossip propagation. The protocol compensates through WoT-targeted forwarding (gossip is directed at likely holders, not broadcast randomly) and relay fallback (Tier~4) which handles 6--21\% of reads depending on topology and network maturity.
...
Rate limiting uses a sliding window counter per source. Excess requests are silently dropped. Combined with WoT-only forwarding, this bounds the gossip blast radius to the trust network while achieving high-probability delivery within it. Gossip provides probabilistic, not guaranteed, delivery. The relay fallback (Tier~4) is an integral component of the delivery mechanism that handles 0.4--13\% of reads at network maturity (day~20--30) depending on graph topology---as low as 0.4\% for sparse hub-structured graphs, rising to 8--13\% for dense or small-world topologies. Relay fallback is not a path of last resort but a permanent component of the delivery architecture.
...
The above calculation assumes independent failures across pact partners. In practice, timezone correlation (pact partners in the same timezone sleeping simultaneously) and community correlation (shared ISP, shared OS updates) introduce correlated failure modes. Discrete-event simulation across multiple graph topologies at 2,000 nodes provides empirical availability figures:
...
\begin{tabular}{lcccc}
\toprule
\textbf{Topology} & \textbf{Availability} & \textbf{Instant} & \textbf{Relay} & \textbf{Failed} \\
\midrule
BA $m{=}10$ (sparse) & 98.4\% & 91.8\% & 6.4\% & 1.6\% \\
WS $p{=}0.30$ (small-world) & 97.7\% & 77.9\% & 9.0\% & 2.3\% \\
BA $m{=}50$ (dense) & 95.3\% & 76.5\% & 18.8\% & 4.7\% \\
BA $m{=}50$ + timezone & 94.8\% & 73.7\% & 21.1\% & 5.2\% \\
\bottomrule
\end{tabular}
\caption{Simulated content availability across graph topologies (2,000 nodes, 30-day runs, seed=42). Sparser graphs with clearer hub structure outperform dense graphs. Timezone correlation reduces availability by ${\sim}0.5\%$.}
\end{table}

Simulated availability (94.8--98.4\%) is substantially below the independent-failure theoretical bound ($10^{-9}$) and below the earlier correlated-failure estimate of 99.9--99.97\%, primarily because pact churn (formation and dissolution dynamics) creates transient periods of reduced redundancy not captured by static probability models. Availability improves sharply with pact maturity: day~1--5 availability is 89--94\%, rising to 97--99.9\% by day~20--30 across all topologies. The protocol recommends geographic diversity in pact partner selection to mitigate timezone correlation.
...
The challenge-response mechanism draws from Filecoin's Proof of Replication and Proof of Spacetime~\cite{filecoin}, and Arweave's Succinct Proofs of Random Access~\cite{arweave}. Our mechanism proves that the pact partner \emph{possesses} the data at challenge time. Unlike Filecoin's PoRep, it does not prove continuous or dedicated storage---a malicious partner could theoretically re-fetch data before responding. The 500ms latency check on serve challenges is a weak heuristic against this strategy. This is a deliberate trade-off: social trust (WoT peers with relationship incentive) reduces the need for cryptoeconomic enforcement, at the cost of being less robust against adversarial pact partners than Filecoin's scheme.
...
\textbf{Redundancy} & Manual multi-relay publishing & None---single home instance & Equilibrium-seeking pact count + 3 standby; simulated availability 94.8--98.4\% across topologies ($10^{-9}$ under theoretical independent-failure assumption) \\
...
The honest gap: our protocol is validated analytically and by discrete-event simulation across multiple graph topologies (Barab\'{a}si--Albert, Watts--Strogatz) at up to 2,000 nodes, including timezone-correlated uptime modeling. Simulation confirms availability of 94.8--98.4\% (below our earlier theoretical estimates of 99.9\%) and shows that pact-local instant reads (74--92\%) dominate over gossip delivery (0.1--9\%). Nostr has ${\sim}1{,}000$ relays and real users. Mastodon has ${\sim}26{,}000$ instances and ${\sim}1.2$M monthly active users. The architectural claims require production-scale validation.
...
The tiered retrieval cascade ensures that the vast majority of reads are served from local pact storage, with relay fallback reserved for cold-start and beyond-graph content. Simulation across Barab\'{a}si--Albert and Watts--Strogatz topologies confirms this: 74--92\% of reads are instant (pact-local), relay dependency decays from 25--44\% (day~1--5) to 0.4--11\% (day~20--30), and sparser hub-structured graphs (BA $m{=}10$, 98.4\% availability) outperform dense graphs (BA $m{=}50$, 95.3\%). The relay is demoted from gatekeeper to optional accelerator, useful during bootstrap and for distant authors, but not required for typical social reading patterns.
...
The hard problems remain. Graph bootstrap for new users requires initial relay dependence and Genesis Guardian infrastructure (see design documentation). A sufficient full-node ratio (at least 5\%) must emerge organically through incentives, not be mandated. DM key rotation provides bounded forward secrecy but not perfect---contingent on correct client implementation of secure key deletion. Social recovery (kinds 10060/10061) is a pre-launch requirement, not a future enhancement: without it, root key loss is permanent identity death. These are engineering challenges, not architectural ones---the social graph is a viable foundation for decentralized infrastructure.
...
\begin{itemize}
  \item \textbf{Minimum viable network size for Phase~2.} At what user count does opportunistic pact retrieval deliver measurably better failure rates than relay-only? Simulation at 1,000 and 2,000 nodes shows the pact benefit emerges quickly: day~1--5 availability is 89--94\%, rising to 97--99\% by day~10--20, with pact-local reads dominating after the first week. Real-world network density may still differ from graph models.
  ...
  \item \textbf{The lurker problem.} In typical social networks, 60--80\% of users are consumers who produce little or no content. These lurkers have minimal storage to offer pact partners, effectively excluding them from the bilateral pact economy. The protocol's plausibility analysis models the full user population, but the effective pact economy may operate over only 20--40\% of users---the active content creators. Lurkers would continue operating in a relay-dependent mode indefinitely. This is not necessarily a failure: the protocol still improves outcomes for the active minority, and lurkers retain exactly the experience they have today on vanilla Nostr.

  \item \textbf{The cooperative reach gradient is unquantified.} The cooperative equilibrium assumes that more pacts yield measurably more reach---incentivizing cooperation over defection. However, the actual magnitude of this gradient has not been measured. Simulation shows that availability ranges from 89\% (day~1--5) to 99\% (day~20--30), suggesting a meaningful but modest benefit curve. Whether a cooperator with 20 pacts gets 5\% or 50\% more reach than a defector with 5 pacts determines whether the cooperative equilibrium is robust or fragile. This differential needs explicit measurement.
  ...
  \item \textbf{Cooperative equilibrium fragility and pact churn.} The cooperative equilibrium is fragile above approximately 30\% defection. Simulation reveals a more concrete concern: in all tested topologies, pact churn is \emph{net-negative}---more pacts dissolve than form over 30 days (churn rates of 2.8--8.0 pacts/node/day). This suggests the pact economy contracts rather than stabilizes, at least within the simulation's volume-matching and reliability-scoring dynamics. Possible explanations include overly aggressive volume-tolerance enforcement ($\delta{=}0.30$), reliability score thresholds that trigger premature drops, or the simulation's uniform activity model failing to capture the stability of real-world friendships. If the contraction persists in production, pact counts would eventually stabilize at a lower equilibrium than the comfort condition predicts, with correspondingly higher relay dependence.
  ...
  \item \textbf{Relay collusion threat.} Colluding relay operators controlling 60--80\% of relay traffic can reconstruct near-complete social graphs from merged metadata, identify probable pact pairings through NIP-46 timing analysis, and reverse all rotating request tokens for known pubkeys. The protocol's primary defense is relay diversity (users connecting through many relays), but whether real-world relay concentration permits this diversity is an empirical question.
\end{itemize}
